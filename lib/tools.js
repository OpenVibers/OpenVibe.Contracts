 ...
 *
 *   contracts.tools.checkDescriptor(d)   -> { valid, errors: [{ path, message }] }, examples checked too
...
const accepts = (accept, mime) => accept === mime || (accept.endsWith('/*') && mime.startsWith(accept.slice(0, -1)));

// A tool's own input schema, compiled once per distinct schema to check its examples. It is the
// tool's JSON Schema, not a contract, so it gets its own lenient validator; its $id is dropped so two
// descriptors naming the same $id never clash.
let inputAjv = null;
const inputValidators = new Map();
const MAX_INPUT_VALIDATORS = 500;
function inputValidator(schema) {
    const key = JSON.stringify(schema);
    let v = inputValidators.get(key);
    if (!v) {
        if (!inputAjv) { inputAjv = new Ajv2020({ allErrors: true, strict: false, allowUnionTypes: true, logger: false }); addFormats(inputAjv); }
        const own = { ...schema };
        delete own.$id;
        try { v = { fn: inputAjv.compile(own), schema: own }; } catch (e) { v = { error: e.message }; }
        if (inputValidators.size >= MAX_INPUT_VALIDATORS) {
            const [oldKey, old] = inputValidators.entries().next().value;
            inputValidators.delete(oldKey);
            if (old.schema) inputAjv.removeSchema(old.schema);
        }
        inputValidators.set(key, v);
    }
    return v;
}

/** An example runs as it says: its input matches an embedded input schema and maxInputBytes, its files fit files. */
function checkExamples(d, err) {
    if (!d.examples) return;
    const check = d.input && !isRefOnly(d.input) ? inputValidator(d.input) : null;
    if (check && check.error) err('/input', `does not compile, so its examples cannot be checked: ${check.error}`);
    d.examples.forEach((ex, i) => {
        const at = `/examples/${i}`;
        if (check && check.fn && !check.fn(ex.input)) for (const e of check.fn.errors) err(`${at}/input${e.instancePath}`, `${e.message} (the tool's input schema)`);
        const bytes = Buffer.byteLength(JSON.stringify(ex.input));
        if (d.limits.maxInputBytes && bytes > d.limits.maxInputBytes) err(`${at}/input`, `is ${bytes} bytes as JSON, over limits.maxInputBytes (${d.limits.maxInputBytes})`);
        if (!ex.files) return;
        if (!d.files) { if (ex.files.length) err(`${at}/files`, 'the tool takes no files'); return; }
        if (ex.files.length < d.files.min || ex.files.length > d.files.max) err(`${at}/files`, `${ex.files.length} files; the tool takes ${d.files.min} to ${d.files.max}`);
        ex.files.forEach((f, j) => { if (!d.files.accept.some(a => accepts(a, f.mime))) err(`${at}/files/${j}/mime`, `${f.mime} is not in files.accept`); });
    });
}
