 ...
 */
function verifyServiceToken(token, { publicKey, issuer, audience, clockSkewSec = 30, now = Date.now() } = {}) {
...
function requireCapability(capability, opts = {}) {
    ...
    return function capabilityGuard(req, res, next) {
        ...
        if (auth.startsWith('Bearer ')) {
            ...
            const r = verifyServiceToken(auth.slice(7).trim(), { publicKey, issuer: opts.issuer, audience: opts.audience });
