 ...
 *   staff.can(roleOrClaims, capability)  exact id or a family grant ('staff.moderation.*')
 */
const fs = require('fs');
...
function can(v, capability) {
    if (!byId.has(capability)) throw new Error(`unknown staff capability ${capability}`);
    const issued = v && typeof v === 'object' && Array.isArray(v[MAP.claims.capabilities]) ? v[MAP.claims.capabilities] : null;
    const held = issued || capabilitiesOf(v);
    return held.some(g => g === capability || (g.endsWith('.*') && capability.startsWith(g.slice(0, -1)) && g.split('.').length === 3));
}
